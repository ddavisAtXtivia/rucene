...
use std::arch::aarch64::{
    vgetq_lane_s32, int32x4_t, vdupq_n_s32, vst1q_s32,
};
#[cfg(target_arch = "arm")]
use std::arch::arm::{
    vgetq_lane_s32, int32x4_t, vdupq_n_s32, vst1q_s32,
...
impl SIMD128Packer {
    ...
    #[allow(dead_code)]
    fn print_m128i_as_i32(v: int32x4_t) {
        unsafe {
            let data = [0i32; 4];
            vst1q_s32(data.get_mut(), v);
            println!("{:?}", data);
        }
    }
}
...
macro_rules! pack_bits {
    ($data: expr, $encoded_data: expr, $num: literal $(,$transfer: expr, $obj: expr)?) => {
        unsafe {
            let mut _input = $data.as_ptr() as *const int32x4_t;
            let mut _output = $encoded_data.as_mut_ptr() as *mut int32x4_t;
            let mut _tmp_value = 0i32;
            let mut _buffer = vld1q_s32(&_tmp_value as *const i32);
            unroll! {
                for i in 0..32 {
